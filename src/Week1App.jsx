...
// MAIN COMPONENT
// =============================================================
export default function Week1App({ onBack, lang: langProp, onLangChange }) {
  const [lang, setLangState] = useState(langProp || "ko");
  const [tab, setTab] = useState("overview");
  // stay in sync if the hub changes language
  useEffect(() => { if (langProp) setLangState(langProp); }, [langProp]);
  const setLang = (l) => {
    setLangState(l);
    if (onLangChange) onLangChange(l); // propagate so the hub (and localStorage) follow
  };
  const t = i18n[lang];
